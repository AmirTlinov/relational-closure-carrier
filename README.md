...
A minimal computational experiment asking whether two body–world histories,
retained as changed relational placement, remain distinguishable in later
repair—without a complete-body target register, reward, or online external
selection inside the modeled loop.
...
> In this prepared digital carrier, two histories of contact with a separately instantiated, independently stateful `WORLD` are retained as different relational placements in `BODY`; they remain distinguishable in later repair after world deletion, body-only restart, common washout, and renewed injury.
...
## Body–world provenance chain

```text
local BODY propagation
        ↓
actual change to an independent WORLD site
        ↓
return from that changed site into BODY
        ↓
changed relational placement survives without WORLD
        ↓
...
- current test suite: 21 passing tests.
